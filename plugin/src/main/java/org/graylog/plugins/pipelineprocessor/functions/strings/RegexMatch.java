...
public class RegexMatch extends AbstractFunction<RegexMatch.RegexMatchResult> {
	...
	public FunctionDescriptor<RegexMatchResult> descriptor() {
		return FunctionDescriptor.<RegexMatchResult> builder().name(NAME).pure(true).returnType(RegexMatchResult.class).params(of(pattern, value, optionalGroupNames))
				.description("Match a string with a regular expression (Java syntax)").build();
	}

	/**
	 * The bean returned into the rule engine. It implements Map so rules can access it directly. <br>
	...
	public static class RegexMatchResult extends ForwardingMap<String, String> {
		...
		public RegexMatchResult(Matcher matcher, List<String> groupNames) {
			...
			while (matcher.find()) {
				...
				final String groupName = Iterables.get(groupNames, i, null);
				builder.put((groupName != null ? groupName : String.valueOf(i)), groupValue);
				i++;
		 ...
		 * @param matchResult
		 * @param groupNames
		 * @deprecated related to issue 173
		 */
